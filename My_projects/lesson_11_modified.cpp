...
int main(int argc, char **argv) {
    ...
    target.arch = Target::ARM;                  // The CPU architecture
    target.bits = 32;                           // The bit-width of the architecture
    std::vector<Target::Feature> arm_features1;  // A list of features to set
    target.set_features(arm_features1);
    ...
    target.arch = Target::ARM;                  // The CPU architecture
    target.bits = 32;                           // The bit-width of the architecture
    std::vector<Target::Feature> arm_features2;  // A list of features to set
    target.set_features(arm_features2);
    // We then pass the target as the last argument to compile_to_file.
    brighter.compile_to_file("lesson_11_arm_32_linux", args, "brighter", target);
	uint8_t arm_32_linux_magic[] = {0x7f, 'E', 'L', 'F',  // ELF format
                                      1,                    // 32-bit
                                      1,                    // 2's complement little-endian
                                      1};                   // Current version of elf

    f = fopen("lesson_11_arm_32_linux.o", "rb");
    ...
    if (memcmp(header, arm_32_linux_magic, sizeof(arm_32_linux_magic))) {
        printf("Unexpected header bytes in 32-bit arm object file.\n");
        return -1;
    }
